...
// Merge flat rows [{date, bank, credit, debit, investment}] into
// [{date, Credit, Debit, Investment, banks: [{bank, credit, debit, investment}]}]
function mergeRows(flat) {
  const byDate = {};
  for (const r of flat) {
    if (!byDate[r.date]) byDate[r.date] = { date: r.date, Credit: 0, Debit: 0, Investment: 0, banks: {} };
    byDate[r.date].Credit += r.credit;
    byDate[r.date].Debit  += r.debit;
    byDate[r.date].Investment += r.investment;
    const b = r.bank || "Unknown";
    if (!byDate[r.date].banks[b]) byDate[r.date].banks[b] = { bank: b, credit: 0, debit: 0, investment: 0 };
    byDate[r.date].banks[b].credit += r.credit;
    byDate[r.date].banks[b].debit  += r.debit;
    byDate[r.date].banks[b].investment += r.investment;
  }
  return Object.values(byDate)
    .sort((a, b) => a.date > b.date ? 1 : -1)
    .map(d => ({
      ...d,
      banks: Object.values(d.banks).sort((a,b) => (
        b.credit + b.debit + b.investment
      ) - (
        a.credit + a.debit + a.investment
      )),
    }));
}
